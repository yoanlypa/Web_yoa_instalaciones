export const siteConfig = {
  businessName: "Yoa Instalaciones",
  tagline: "Reparaciones e instalaciones hechas con cuidado",
  description:
    "Montador de muebles e instalador en Málaga capital, Fuengirola, Mijas y alrededores: montaje de muebles, lámparas, ventiladores de techo, tendederos y grifos. Pide tu presupuesto por WhatsApp.",
  whatsappNumber: "34603379870",
  phoneDisplay: "+34 603 37 98 70",
  instagramUrl: "https://instagram.com/",
  facebookUrl: "https://facebook.com/",
  taskiaUrl: "https://www.taskia.es/t/496139",
  location: "España",
};

export const services = [
  {
    title: "Montaje de muebles",
    description:
      "Montaje de muebles de cualquier tipo (IKEA y otras marcas), listos y bien nivelados.",
  },
  {
    title: "Iluminación y ventilación",
    description:
      "Instalación de lámparas, apliques y ventiladores de techo.",
  },
  {
    title: "Fijación en pared",
    description:
      "Estanterías flotantes, soportes de TV, toalleros y accesorios de baño, bien anclados.",
  },
  {
    title: "Persianas, estores y tendederos",
    description:
      "Instalación de persianas, estores y tendederos exteriores e interiores.",
  },
  {
    title: "Grifos y electricidad básica",
    description: "Cambio e instalación de grifos, enchufes e interruptores.",
  },
  {
    title: "Carpintería e instalaciones a medida",
    description:
      "Puertas, paneles y mobiliario de madera fabricado e instalado a medida.",
  },
  {
    title: "Presupuesto personalizado",
    description:
      "Cuéntame lo que necesitas y te preparo un presupuesto ajustado, sin compromiso.",
  },
];

export const reviews = [
  {
    author: "Juan Carlo T.",
    rating: 5,
    text: "Yoanly fue fantástico. Llegó puntual y con todas sus herramientas. Es preciso y metódico: instaló ventiladores de techo, cambió lámparas de techo e instaló un grifo a la perfección, limpiando todo a medida que avanzaba. Me sugirió mejores soluciones en varias ocasiones. Lo recomiendo total y volveré a contratarlo cuando necesite realizar algún trabajo.",
  },
  {
    author: "Diana G.",
    rating: 5,
    text: "Excelente actitud. Él y su ayudante fueron muy profesionales, con gran conocimiento de lo que estaban haciendo. Muy amables, puntuales y resolutivos. Nosotros no lográbamos hacer que funcionara la instalación del soporte de la TV, pero él lo solucionó utilizando sus propias herramientas. Realmente hizo que todo funcionara y quedé muy impresionada.",
  },
  {
    author: "María M.",
    rating: 5,
    text: "Me ha encantado el trabajo de Yoanli. Llegó puntual y realizó el trabajo de forma excelente, dejando todo limpio al final. Lo recomiendo al 100% y no dudaré en solicitar sus servicios más adelante.",
  },
  {
    author: "Carmen G.",
    rating: 5,
    text: "Yoanly es un excelente profesional, muy amable y cuidadoso. Nos montó el mueble a la perfección. Hemos quedado muy satisfechos con su trabajo.",
  },
  {
    author: "Ghadeer A.",
    rating: 5,
    text: "Yoanly deserves more than 5 stars.. he's so professional and polite.. I highly recommend him.. thank you so much Yoanly.",
  },
  {
    author: "Rosa B.",
    rating: 5,
    text: "Yoanly ha sido puntual y es un gran profesional.",
  },
];

export const gallery = [
  {
    type: "image",
    src: "/images/trabajos/mueble-tv-chimenea.jpg",
    alt: "Mueble de TV a medida con chimenea integrada y estanterías con iluminación",
    caption: "Trabajo de carpintería a medida",
  },
  {
    type: "image",
    src: "/images/trabajos/rascador-gatos-medida.jpg",
    alt: "Rascador y mobiliario para gatos hecho a medida junto a una ventana",
    caption: "Montaje de todo tipo de mobiliario",
  },
];
